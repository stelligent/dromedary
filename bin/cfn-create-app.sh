...
ssh_location="$(aws cloudformation describe-stacks --stack-name $dromedary_master_stack_name --output text --query 'Stacks[0].Outputs[?OutputKey==`SSHLocation`].OutputValue')"
...
aws cloudformation create-stack \
    ...
    --parameters ParameterKey=Ec2Key,ParameterValue=$dromedary_ec2_key \
        ...
        ParameterKey=SshCidr,ParameterValue=$ssh_location \
