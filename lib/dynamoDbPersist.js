...
function Constructor() {
  ...
  this.init = function (cb) {
    ...
    if (process.env.hasOwnProperty('DROMEDARY_DDB_TABLE_NAME')) {
      ...
      console.log("Loading from DynamoDB table '"+ddbTableName+"' in region '"+AWS.config.region+"'");
      ddb = new AWS.DynamoDB();
      ddb.waitFor('tableExists', { TableName: ddbTableName }, cb);
    } else {
      console.log("Loading from *LOCAL* DynamoDB table '"+dromedary_dev+"' at endpoint 'http://localhost:8079'");
